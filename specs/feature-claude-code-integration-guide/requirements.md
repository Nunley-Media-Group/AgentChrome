...
- Then: they see: connect → navigate → snapshot → interact → verify with concrete agentchrome commands
...
**Given** an AI agent using agentchrome encounters an error
...
**Given** an AI agent developer wants to optimize agentchrome usage
...
| Aspect | Requirement |
|--------|-------------|
| **Accuracy** | All commands shown must match the current agentchrome CLI interface |
| **Readability** | Written for both human developers and AI agents to parse |
| **Maintainability** | Commands should reference `agentchrome capabilities` and `agentchrome examples` so the guide stays up-to-date |
...
- [x] Core agentchrome commands implemented (connect, navigate, page, interact, form, etc.)
