...
- `dom select`
- `dom attributes` (i.e. the `dom get-attribute` / full-attribute read path)
...
| `dom attributes` | `{attribute_count, keys_seen}` |
...
| FR4 | `output::emit` gating added to `dom select`, `dom attributes`, `dom events` | Must | See AC4 for each summary shape |
