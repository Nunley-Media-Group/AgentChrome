...
#
# Generated from: specs/5-chrome-instance-discovery-and-launch/requirements.md
# Issue: #5
...
Feature: Chrome instance discovery and launch
  ...
  Scenario: Connect via direct WebSocket URL
    Given agentchrome is built
    When I run "agentchrome --ws-url ws://127.0.0.1:9222/devtools/browser/test-id connect"
    Then the exit code should be 0
  ...
  Scenario: WebSocket URL port is extracted correctly
    Given agentchrome is built
    When I run "agentchrome --ws-url ws://127.0.0.1:9333/devtools/browser/abc connect"
    Then the exit code should be 0
    And stdout should be valid JSON
  ...
  Scenario: Connect with no Chrome running produces error JSON (AC13)
    Given agentchrome is built
    When I run "agentchrome --port 19222 connect"
  ...
  Scenario: Launch with invalid chrome-path produces error
    Given agentchrome is built
    When I run "agentchrome connect --launch --chrome-path /nonexistent/chrome"
    Then the exit code should be nonzero
    And stderr should be valid JSON
    And stderr JSON should have key "error"
  ...
  Scenario: Headless requires launch flag
    Given agentchrome is built
    When I run "agentchrome connect --headless"
  ...
  Scenario: Channel requires launch flag
    Given agentchrome is built
    When I run "agentchrome connect --channel canary"
    Then the exit code should be 1
    And stderr should be valid JSON
    And stderr JSON should have key "error"
    And stderr JSON should have key "code"
  ...
  Scenario: Chrome-path requires launch flag
    Given agentchrome is built
    When I run "agentchrome connect --chrome-path /some/path"
    Then the exit code should be 1
    And stderr should be valid JSON
    And stderr JSON should have key "error"
    And stderr JSON should have key "code"
  ...
  Scenario: Chrome-arg requires launch flag
    Given agentchrome is built
    When I run "agentchrome connect --chrome-arg=--disable-gpu"
    Then the exit code should be 1
    And stderr should be valid JSON
    And stderr JSON should have key "error"
    And stderr JSON should have key "code"
  ...
  Scenario: Non-localhost host emits warning
    Given agentchrome is built
    When I run "agentchrome --host 192.168.1.100 --ws-url ws://192.168.1.100:9222/test connect"
