...
- Add `agentchrome markdown` for clean HTML-to-Markdown conversion across page, file, stdin, and URL sources with deterministic cleanup, structured JSON/plain output, and focused BDD coverage. (#269)
...
- Regenerate packaged man pages after the 1.51.x patch releases so the committed manual pages and binary version remain deterministic in CI.
