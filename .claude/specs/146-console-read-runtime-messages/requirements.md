...
**Given** an agent has run `agentchrome js exec "console.log('test'); console.error('oops')"` in a prior CLI invocation
**When** I run `agentchrome console read` in a subsequent, independent CLI invocation
**Then** the output includes both the log and error messages from the prior invocation

**Example**:
- Given: `agentchrome js exec "console.log('test'); console.error('oops')"` completed successfully
- When: `agentchrome console read`
...
**And when** I run `agentchrome console read --type warn`
**Then** only warn-level messages are returned

**And when** I run `agentchrome console read --limit 2`
...
Feature: Console Read Runtime Messages
  ...
  Scenario: Console read captures messages from js exec across invocations
    ...
    When I run "agentchrome console read" in a new invocation
