...
Feature: Navigate back/forward/reload respects global --timeout option
  ...
  ignored the global --timeout flag and AGENTCHROME_TIMEOUT environment variable,
  ...
  @regression
  Scenario: AC4 — AGENTCHROME_TIMEOUT environment variable works for history navigation
    Given a tab with navigation history
    And AGENTCHROME_TIMEOUT is set to "5000"
  ...
  Scenario: AC5 — default timeout preserved when no override specified
    Given a tab with navigation history
    When I run "agentchrome navigate back" with no --timeout flag or AGENTCHROME_TIMEOUT set
